...
const Card = () => {
      ...
      try {
        const response = await fetch(
          "https://api.weekday.technology/adhoc/getSampleJdJSON", //API : DATA FETCHED
          {
            ...
            body: JSON.stringify({
              limit: 10, // Set Limit to Fetch
              offset: 0, //Start form Zero
            }),
          }
        );
        if (!response.ok) {
  ...
  // To Capitalize First Word of Every Letter :: For Data Coming from API
  ...
  const Modal = ({ onClose, jobDetails }) => {
    return (
  ...
  const minSALARY = eventsData.maxJdSalary;
    ...
    <>
      <div className="card-container">
        {eventsData.map(
          (
            event,
            index // Looping the Data from Array sent by API
          ) => (
                  ...
                  <div className="company-details">
                    {event.jobDetailsFromCompany.length > 200 ? (
                      <>
                        ...
                        <button onClick={() => handleShowMore(event)}>
                          Show more
                        </button>
                      </>
                    ) : (
                ...
                <button className="card-button">
                  <svg
                    ...
                    <g
                      ...
                      stroke-linejoin="round"
                      stroke="#CCCCCC"
                      stroke-width="0.44000000000000006"
                    />
      ...
      {selectedJob && ( //Modal is conditionally Rendered
        <Modal
